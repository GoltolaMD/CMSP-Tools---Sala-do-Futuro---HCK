javascript:(()=>{
  ...
  const THEMES = {
    dark: `#cmsp-tools{background:#1e1e1e;color:#f1f1f1}#cmsp-tools a{background:#2c2c2c;color:#fff}#cmsp-tools a:hover{background:#3d3d3d}`,
    light: `#cmsp-tools{background:#fff;color:#222}#cmsp-tools a{background:#f0f0f0;color:#000}#cmsp-tools a:hover{background:#e0e0e0}`,
    colorful: `#cmsp-tools{background:linear-gradient(135deg,#ff69b4,#8e24aa);color:#fff}#cmsp-tools a{background:rgba(255,255,255,0.1);color:#fff}#cmsp-tools a:hover{background:rgba(255,255,255,0.2)}`
  };
  ...
  const baseStyles = `
    ...
    #cmsp-tools h3 { margin: 0 0 10px; font-size: 18px; cursor: move; }
    #cmsp-tools button {
      margin-left: 5px;
  ...
  box.id = 'cmsp-tools';
  box.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;cursor:move;" id="cmsp-drag">
      <h3>📦 CMSP Tools</h3>
      <div>
        <button onclick="document.getElementById('cmsp-content').style.display = (document.getElementById('cmsp-content').style.display === 'none') ? 'block' : 'none'">➖</button>
    ...
    <div id="cmsp-content">
      <div style="margin: 10px 0;">
        ...
        <button onclick="(${applyTheme.toString()})('colorful')" style="background:linear-gradient(45deg,#ff69b4,#8e24aa);color:#fff;">🌈 Colorido</button>
      </div>
      <div>
  ...
  function makeDraggable(el) {
    ...
    const dragEl = document.getElementById("cmsp-drag");
    if (!dragEl) return;
